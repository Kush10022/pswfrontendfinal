...
import PswAvailabilityProfileCalendar from "../lib/profileCalander";
  ...
  const retriveUser = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(
          responseData.message || "Failed to fetch user profile metadata"
        );
      }
    } catch (error) {
      ...
      <Container>
        <Row>
          <Col md={5} className={styles.leftColumn}>
            ...
            <div className={styles.userInfo}>
              ...
              {userProfile?.user.isPSW ? (
                <p>You are registered as a PSW</p>
              ) : (
                <p>You are not registered as a PSW</p>
              )}
            </div>
            <Button
              variant="success"
              className=" mt-4 py-2 ml-12  btn-success bg-success"
        ...
        <div>
          ...
          <Modal open={open} onClose={onCloseModal} center>
            <PswAvailabilityProfileCalendar />
          </Modal>
        </div>
      </Container>
    </div>
